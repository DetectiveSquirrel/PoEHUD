...
namespace PoeHUD.Hud.KillCounter
{
    public class KillCounterPlugin : SizedPlugin<KillCounterSettings>
    {
            ...
            : base(gameController, graphics, settings)
        {
            ...
            MonstersController.Current.OnMonsterDeath += OnMonsterDeath;
        }
        ...
        private void OnMonsterDeath(MonsterDeathParams deathInfo)
        {
            if (deathInfo.KilledByOtherPlayer) //if we want to include info that s1 killed an enemy
                return;

            if (deathInfo.MonsterEntity.IsAlly)
                return;

            var rarity = deathInfo.MonsterEntity.GetComponent<ObjectMagicProperties>().Rarity;
